functions {
  vector MSFRm(real t, vector y, vector theta, int m) {
    ...
    real denominator;
    denominator = 1.0;
    for(i in 1:m) denominator = denominator+(theta[i]*theta[m+i]*y[i]^(1.0+theta[1+2*m]));
    for(i in 1:m) dydt[i] = -theta[i]*y[i]^(1.0+theta[1+2*m]) / denominator;
...
model{
  ...
  vector[1+2*m] pars;     // parameters for ODE
...
generated quantities{
  array[nLL] real log_lik;{
    int k=0;
    array[1] vector[m] N_p; // predictions
    vector[2*m+1] pars;     // parameters for ODE
    for (i in 1:m){
      pars[i] = a[i];
      pars[m+i] = h[i];
    }
